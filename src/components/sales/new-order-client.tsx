...
export function NewOrderClient({ orders, institutions, drugs }: NewOrderClientProps) {
    ...
    const handleDrugChange = (id: string, field: keyof DrugItem, value: string | number) => {
        setDrugItems(drugItems.map(item => {
            if (item.id === id) {
        ...
        <div className="flex flex-col h-full p-6 space-y-4 overflow-hidden">
            <div className="flex items-center justify-between flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white shadow-md">提交</Button>
                </div>
            </div>
                        ...
                        <Select value={selectedInstitution} onValueChange={setSelectedInstitution}>
                            ...
                            <SelectContent>
                                {institutions.map(inst => (
                                    <SelectItem key={inst.approval_no} value={inst.approval_no}>{inst.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    {drugItems.length === 0 ? (
                        ...
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 bg-slate-100 dark:bg-slate-800 z-10">
                                <tr>
                                    <th className="px-4 py-3.5 text-left font-semibold text-slate-700 dark:text-slate-200 text-sm">药品批准号</th>
                                    ...
                                    <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">数量</th>
                                    <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">单价</th>
                                    <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">金额</th>
                                    <th className="px-4 py-3.5 text-center font-semibold text-slate-700 dark:text-slate-200 text-sm w-20">操作</th>
                                </tr>
                            </thead>
