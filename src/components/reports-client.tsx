...
export default function ReportsClient({ initialData }: ReportsClientProps) {
    ...
    const filteredData = useMemo(() => {
        ...
        switch (activeReport) {
            case 'manufacturer':
                return initialData.manufacturers.filter(item =>
            ...
            case 'drug':
                return initialData.drugs.filter(item =>
            ...
            case 'institution':
                return initialData.institutions.filter(item =>
            ...
            case 'purchase':
                return initialData.purchaseOrders.filter(item =>
            ...
            case 'sales':
                return initialData.salesOrders.filter(item =>
            ...
            case 'purchase_storage':
                return initialData.purchaseStorages.filter(item =>
                    !q || item.orderNo.toLowerCase().includes(q) || item.drug_name.toLowerCase().includes(q) || item.warehouse_code.toLowerCase().includes(q)
                );
            case 'sales_outbound':
                return initialData.salesOutbounds.filter(item =>
                    !q || item.orderNo.toLowerCase().includes(q) || item.drug_name.toLowerCase().includes(q) || item.warehouse_code.toLowerCase().includes(q)
                );
            case 'inventory':
                return initialData.inventory.filter(item =>
    ...
    const tableColumns = useMemo((): ColumnDef[] => {
        const baseColumns: Record<ReportType, ColumnDef[]> = {
            ...
            inventory: [
                ...
                { key: 'drug_name', label: '药品名称', render: (v) => <span className="font-medium text-slate-800 dark:text-slate-100">{v}</span> },
                { key: 'quantity', label: '库存数量', render: (v) => <span className='inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-teal-50 dark:bg-teal-950/30 text-teal-700 dark:text-teal-300 font-bold text-sm'>{v}<Sparkles className='w-3 h-3 opacity-50' /></span> },
    ...
    const rowKey = useCallback((item: any): string | number => {
        switch (activeReport) {
            case 'manufacturer':
            case 'drug':
            case 'institution':
            ...
            case 'sales_outbound':
                return (
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3.5'>
                        ...
                        <DetailItem label="出库数量" value={<span className="font-bold text-teal-600 dark:text-teal-400">{selectedItem.quantity}</span>} />
                        <DetailItem label="销售员" value={selectedItem.salesperson} />
                        <DetailItem label="检验员" value={selectedItem.inspector} />
                        <DetailItem label="保管员" value={selectedItem.keeper} />
                    </div>
            ...
            case 'inventory':
                return (
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3.5'>
                        ...
                        <DetailItem label="库存数量" value={<span className='inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-teal-50 dark:bg-teal-950/30 text-teal-700 dark:text-teal-300 font-bold'>{selectedItem.quantity}<Sparkles className='w-3 h-3 opacity-50' /></span>} />
                ...
                <div className='px-6 py-4'>
                    <div className='flex items-center justify-between mb-4'>
                        <div>
                            ...
                            <p className='text-sm text-slate-500 dark:text-slate-400 mt-1'>实时查看企业运营数据，支持多维度筛选与导出</p>
                        </div>
                        <Button
