    ...
    return (
        <div className="flex flex-col h-full p-6 space-y-4 overflow-hidden">
            <div className="flex items-center justify-between flex-shrink-0">
                ...
                <Button className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white shadow-md">确认退货</Button>
            </div>
            ...
            <div className="flex-shrink-0 bg-white dark:bg-slate-800/60 rounded-xl border border-slate-200/60 dark:border-slate-700/40 p-4">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Select value={selectedInstitution} onValueChange={(v) => {
                        ...
                        <Select value={selectedOrderNo} onValueChange={setSelectedOrderNo} disabled={!selectedInstitution}>
                            ...
                            <SelectContent>
                                {filteredOrders.map(order => (
                                    <SelectItem key={order.order_no} value={order.order_no}>{order.order_no}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                ...
                {!selectedOrder ? (
                    ...
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-slate-100 dark:bg-slate-800 z-10">
                            <tr>
                                <th className="px-4 py-3.5 text-center font-semibold text-slate-700 dark:text-slate-200 text-sm w-20">选择</th>
                                ...
                                <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">已售量</th>
