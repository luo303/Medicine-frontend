    ...
    return (
        <div className="flex flex-col h-full p-6 space-y-4 overflow-hidden">
            <div className="flex items-center justify-between flex-shrink-0">
                ...
                <Button className="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white shadow-md">确认出库</Button>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-slate-100 dark:bg-slate-800 z-10">
                            <tr>
                                ...
                                <th className="px-4 py-3.5 text-left font-semibold text-slate-700 dark:text-slate-200 text-sm">有效期</th>
                                <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">销售量</th>
                                <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">已出库</th>
                                <th className="px-4 py-3.5 text-center font-semibold text-slate-700 dark:text-slate-200 text-sm">本次出库</th>
                            ...
                            {selectedOrder.salesDetails.map((detail) => {
                                ...
                                return (
                                    <tr key={detail.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20">
                                        ...
                                        <td className="px-4 py-3 font-mono text-sm">B{detail.production_date.replace(/-/g, '').slice(2)}001</td>
                ...
                {selectedOrder && selectedOrder.salesDetails.length > 0 && (
                    <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
                        ...
                        <p className="text-sm text-amber-600/80 dark:text-amber-400/80 mt-1">
                            建议优先出库批号 B{selectedOrder.salesDetails[0].production_date.replace(/-/g, '').slice(2)}001（有效期 2025-12-31）
                        </p>
                    </div>
                )}
            </div>
