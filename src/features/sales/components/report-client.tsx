                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Select value={year} onValueChange={setYear}>
                            ...
                            <SelectContent>
                                {years.map(y => (
                                    <SelectItem key={y} value={y}>{y === '全部' ? '全部' : `${y}年`}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead className="sticky top-0 bg-slate-100 dark:bg-slate-800 z-10">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">销售单数</th>
                                            <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">销售额</th>
                                            <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">成本</th>
                                            <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">毛利</th>
                                            <th className="px-4 py-3.5 text-right font-semibold text-slate-700 dark:text-slate-200 text-sm">毛利率</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/40">
                                        {monthlyData.map((data) => (
                                            <tr key={data.month} className="hover:bg-slate-50 dark:hover:bg-slate-700/20">
                                                ...
                                                <td className="px-4 py-3 text-right font-mono text-teal-600">¥{data.salesAmount.toLocaleString()}</td>
                                                <td className="px-4 py-3 text-right font-mono">¥{data.costAmount.toLocaleString()}</td>
                                                <td className="px-4 py-3 text-right font-mono text-teal-600">¥{data.profit.toLocaleString()}</td>
