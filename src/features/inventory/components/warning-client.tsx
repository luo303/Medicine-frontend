...
export function WarningClient({ inventories }: WarningClientProps) {
    ...
    const highStockWarnings = useMemo(() => {
        const maxStock = 5000;
        return inventories
            .filter(item => item.quantity > maxStock)
            .sort((a, b) => b.quantity - a.quantity);
    }, [inventories]);

    const getExpiryStatus = (days: number) => {
        if (days <= 30) return { color: 'bg-red-50 text-red-600 dark:bg-red-900/20', text: '紧急处理', icon: AlertCircle };
        if (days <= 60) return { color: 'bg-amber-50 text-amber-600 dark:bg-amber-900/20', text: '需关注', icon: AlertTriangle };
    ...
    const getStockStatus = (current: number, safety: number) => {
        if (current < safety * 0.5) return { color: 'bg-red-50 text-red-600 dark:bg-red-900/20', text: '严重不足', icon: AlertCircle };
        if (current < safety) return { color: 'bg-amber-50 text-amber-600 dark:bg-amber-900/20', text: '库存偏低', icon: AlertTriangle };
                                        ...
                                        {lowStockWarnings.map((item) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1.5 ${status.color}`}>
