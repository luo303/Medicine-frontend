...
export function CheckClient({ inventories }: CheckClientProps) {
    ...
    const filteredInventories = useMemo(() => {
        return inventories.filter(item => 
            selectedWarehouse === '全部' || item.warehouse.name === selectedWarehouse
        );
                            ...
                            <div className="border rounded-lg overflow-hidden">
                                <table className="w-full">
                                    <thead className="bg-slate-100 dark:bg-slate-800">
                                        <tr>
                                            <th className="px-4 py-2 text-left text-sm font-medium text-slate-600 dark:text-slate-300 w-40">药品名称</th>
                                            <th className="px-4 py-2 text-left text-sm font-medium text-slate-600 dark:text-slate-300 w-32">批号</th>
                                            <th className="px-4 py-2 text-left text-sm font-medium text-slate-600 dark:text-slate-300 w-24">货位</th>
                                            <th className="px-4 py-2 text-right text-sm font-medium text-slate-600 dark:text-slate-300 w-24">账面数量</th>
                                            <th className="px-4 py-2 text-right text-sm font-medium text-slate-600 dark:text-slate-300 w-24">实盘数量</th>
                                            <th className="px-4 py-2 text-right text-sm font-medium text-slate-600 dark:text-slate-300 w-24">差异</th>
                                            <th className="px-4 py-2 text-left text-sm font-medium text-slate-600 dark:text-slate-300">差异原因</th>
                                        </tr>
                                    </thead>
